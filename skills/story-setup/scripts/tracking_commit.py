...
VOLUME_AUDIT_STATUSES = ("pass", "warn", "block")
OPEN_FORESHADOW = "已埋"
...
def normalize_needs_review(value: object, last_chapter: int) -> list[int]:
    chapters = as_list(value if value is not None else [], "tracking state.needs_review")
    normalized: list[int] = []
    seen: set[int] = set()
    for index, raw in enumerate(chapters):
        chapter = as_int(raw, f"tracking state.needs_review[{index}]", minimum=1)
        require(chapter <= last_chapter, f"needs_review chapter {chapter} exceeds last committed chapter")
        require(chapter not in seen, f"needs_review contains duplicate chapter {chapter}")
        seen.add(chapter)
        normalized.append(chapter)
    return sorted(normalized)


def normalize_volume_audits(value: object) -> dict[str, dict[str, Any]]:
    audits = as_mapping(value if value is not None else {}, "tracking state.volume_audits")
    normalized: dict[str, dict[str, Any]] = {}
    for raw_key, raw_audit in audits.items():
        key = clean_text(raw_key, "tracking state.volume_audits key", max_bytes=240)
        audit = as_mapping(raw_audit, f"tracking state.volume_audits.{key}")
        require_known_keys(
            audit,
            {"status", "volume", "start_chapter", "end_chapter", "state_revision", "findings", "note"},
            f"tracking state.volume_audits.{key}",
        )
        status = clean_text(audit.get("status"), f"tracking state.volume_audits.{key}.status", max_bytes=24)
        require(status in VOLUME_AUDIT_STATUSES, f"tracking state.volume_audits.{key}.status is invalid")
        findings = as_list(audit.get("findings", []), f"tracking state.volume_audits.{key}.findings")
        normalized_findings: list[dict[str, Any]] = []
        for index, raw_finding in enumerate(findings):
            finding = as_mapping(raw_finding, f"tracking state.volume_audits.{key}.findings[{index}]")
            require_known_keys(
                finding,
                {"severity", "code", "detail", "id", "chapter"},
                f"tracking state.volume_audits.{key}.findings[{index}]",
            )
            severity = clean_text(finding.get("severity"), f"tracking state.volume_audits.{key}.findings[{index}].severity", max_bytes=24)
            require(severity in {"block", "warn", "info"}, f"tracking state.volume_audits.{key}.findings[{index}].severity is invalid")
            item: dict[str, Any] = {
                "severity": severity,
                "code": clean_text(finding.get("code"), f"tracking state.volume_audits.{key}.findings[{index}].code", max_bytes=80),
            }
            if finding.get("detail") is not None:
                item["detail"] = clean_text(finding.get("detail"), f"tracking state.volume_audits.{key}.findings[{index}].detail", max_bytes=480)
            if finding.get("id") is not None:
                item["id"] = clean_text(finding.get("id"), f"tracking state.volume_audits.{key}.findings[{index}].id", max_bytes=24)
            if finding.get("chapter") is not None:
                item["chapter"] = as_int(finding.get("chapter"), f"tracking state.volume_audits.{key}.findings[{index}].chapter", minimum=1)
            normalized_findings.append(item)
        normalized[key] = {
            "status": status,
            "volume": safe_file_component(audit.get("volume"), f"tracking state.volume_audits.{key}.volume"),
            "start_chapter": as_int(audit.get("start_chapter"), f"tracking state.volume_audits.{key}.start_chapter", minimum=1),
            "end_chapter": as_int(audit.get("end_chapter"), f"tracking state.volume_audits.{key}.end_chapter", minimum=1),
            "state_revision": as_int(audit.get("state_revision"), f"tracking state.volume_audits.{key}.state_revision"),
            "findings": normalized_findings,
            "note": clean_text(audit.get("note"), f"tracking state.volume_audits.{key}.note", max_bytes=480),
        }
    return normalized


def compute_json_volume_audit(
    state: dict[str, Any],
    volume: str,
    start_chapter: int,
    end_chapter: int,
) -> dict[str, Any]:
    require(end_chapter >= start_chapter, "volume audit end chapter precedes start chapter")
    findings: list[dict[str, Any]] = []
    for chapter in state["needs_review"]:
        if start_chapter <= chapter <= end_chapter:
            findings.append({"severity": "block", "code": "needs-review", "chapter": chapter})
    for identifier, row in state["foreshadow"].items():
        if row["status"] != OPEN_FORESHADOW:
            continue
        due = row["planned_resolution_chapter"]
        planted = row["planted_chapter"]
        if due is None:
            continue
        if due <= end_chapter and planted <= end_chapter:
            findings.append(
                {
                    "severity": "block",
                    "code": "overdue-foreshadow",
                    "id": identifier,
                    "chapter": due,
                    "detail": row["summary"],
                }
    ...
    for identifier, event in state["timeline"].items():
        reveal = event.get("reveal_chapter")
        if reveal is not None and reveal > end_chapter:
            findings.append(
                {
                    "severity": "block",
                    "code": "timeline-future-reveal",
                    "id": identifier,
                    "chapter": reveal,
                    "detail": "reveal_chapter is after the audited volume end",
                }
            )
    status = "pass"
    ...
    return {
        ...
        "findings": findings,
        "note": "JSON 热状态审计覆盖到期伏笔、待复核和时间线揭示章；人物弧与字段级依赖只有 SQLite 引擎才记录。文风漂移不作为硬门槛。",
    }


def current_volume_audit(state: dict[str, Any]) -> dict[str, Any] | None:
    position = state["context"]["position"]
    return state["volume_audits"].get(volume_key(position["volume"], position["volume_start_chapter"]))
...
def volume_key(volume: str, start_chapter: int) -> str:
    return f"{start_chapter}:{volume}"


def sqlite_database_path(project: Path) -> Path:
    return tracking_root(project) / "story-state.sqlite3"


def require_json_engine(project: Path) -> None:
    require(
        not sqlite_database_path(project).is_file(),
        "this book uses the SQLite fact source; use story_state.py, do not write _tracking-state.json",
    )
...
def validate_position(value: object, label: str = "context.position") -> dict[str, Any]:
    ...
    require_known_keys(
        position, {"volume", "volume_start_chapter", "story_time", "scene", "planned_end_chapter"}, label
    )
    normalized = {
    ...
    if position.get("planned_end_chapter") is not None:
        planned = as_int(position.get("planned_end_chapter"), f"{label}.planned_end_chapter", minimum=1)
        require(
            planned >= normalized["volume_start_chapter"],
            f"{label}.planned_end_chapter cannot precede volume_start_chapter",
        )
        normalized["planned_end_chapter"] = planned
    return normalized
        ...
        (
            ...
            [
                ...
                (
                    ...
                    + (
                        f"，计划至第{position['planned_end_chapter']}章"
                        if position.get("planned_end_chapter")
                        else ""
                    )
                    + "）"
                ),
...
def normalize_state(document: object) -> dict[str, Any]:
    ...
    require_known_keys(
        root,
        {
            ...
            "wordcount_records", "needs_review", "volume_audits",
        },
        "tracking state",
    )
    ...
    needs_review = normalize_needs_review(root.get("needs_review", []), last_chapter)
    volume_audits = normalize_volume_audits(root.get("volume_audits", {}))
    return {
...
def normalize_transaction(project: Path, state: dict[str, Any], document: object) -> dict[str, Any]:
    ...
    require_known_keys(
        root,
        {
            ...
            "delta", "context", "character_snapshots", "wordcount", "continuity_changed",
        },
        "transaction",
    )
    ...
    continuity_changed = as_bool(root.get("continuity_changed", False), "continuity_changed")
    ...
    if mode == "append":
        ...
        require(not continuity_changed, "continuity_changed is only valid on revision")
        require(
            not state["needs_review"],
            "cannot append while downstream chapters need review: "
            + "、".join(f"第{item}章" for item in state["needs_review"]),
        )
    else:
...
def merge_transaction(state: dict[str, Any], transaction: dict[str, Any]) -> dict[str, Any]:
    ...
    if transaction["mode"] == "revision":
        if transaction["continuity_changed"]:
            needs_review.update(range(chapter + 1, current_last + 1))
        needs_review.discard(chapter)
    next_state["needs_review"] = sorted(needs_review)

    previous_position = state["context"]["position"]
    next_position = next_state["context"]["position"]
    previous_key = volume_key(previous_position["volume"], previous_position["volume_start_chapter"])
    next_key = volume_key(next_position["volume"], next_position["volume_start_chapter"])
    if transaction["mode"] == "append" and previous_key != next_key:
        previous_end = state["last_committed_chapter"]
        if previous_end >= previous_position["volume_start_chapter"]:
            audit = compute_json_volume_audit(
                state,
                previous_position["volume"],
                previous_position["volume_start_chapter"],
                previous_end,
            )
            audit["state_revision"] = next_state["state_revision"]
            next_state["volume_audits"][previous_key] = audit
            require(
                audit["status"] != "block",
                "cannot open a later volume while "
                f"{previous_position['volume']} has unresolved blockers: "
                + "；".join(item["code"] for item in audit["findings"] if item["severity"] == "block"),
            )
    planned_end = next_position.get("planned_end_chapter")
    if (
        transaction["mode"] == "append"
        and planned_end is not None
        and next_state["last_committed_chapter"] >= planned_end
    ):
        next_state["volume_audits"][next_key] = compute_json_volume_audit(
            next_state,
            next_position["volume"],
            next_position["volume_start_chapter"],
            next_state["last_committed_chapter"],
        )
...
def persist_volume_audit(project: Path) -> dict[str, Any]:
    with project_write_lock(project):
        require_json_engine(project)
        ...
        position = state["context"]["position"]
        last = state["last_committed_chapter"]
        require(last >= position["volume_start_chapter"], "current volume has no committed chapters to audit")
        audit = compute_json_volume_audit(state, position["volume"], position["volume_start_chapter"], last)
        next_state = copy.deepcopy(state)
        next_state["state_revision"] += 1
        next_state["volume_audits"][volume_key(position["volume"], position["volume_start_chapter"])] = {
            **audit,
            "state_revision": next_state["state_revision"],
        }
        next_state = normalize_state(next_state)
        views = render_views(next_state)
        write_views(tracking_root(project), views)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    audit_parser = subparsers.add_parser("audit-volume")
    audit_parser.add_argument("--project", type=Path, required=True, help="book project root containing 追踪/")
    audit_parser.add_argument(
        "--persist",
        action="store_true",
        help="write the current-volume audit into _tracking-state.json",
    )
    return parser
...
def compact_result(result: dict[str, Any]) -> dict[str, Any]:
    ...
    audit = current_volume_audit(result)
    if audit is not None:
        payload["volume_audit"] = {
            "status": audit["status"],
            "volume": audit["volume"],
            "start_chapter": audit["start_chapter"],
            "end_chapter": audit["end_chapter"],
...
def main() -> int:
    ...
    try:
        if args.command == "init":
            ...
            if args.persist:
                result = persist_volume_audit(args.project)
            else:
                require_json_engine(args.project)
                result = check_project(args.project)
                position = result["context"]["position"]
                last = result["last_committed_chapter"]
                require(last >= position["volume_start_chapter"], "current volume has no committed chapters to audit")
                audit = compute_json_volume_audit(
                    result, position["volume"], position["volume_start_chapter"], last
                )
                emit(json.dumps({"volume_audit": audit}, ensure_ascii=False))
                return 1 if audit["status"] == "block" else 0
        else:
            result = check_project(args.project)
    except (TrackingError, OSError, UnicodeError) as exc:
        emit(f"ERROR: {exc}", error=True)
        return 2
    emit(json.dumps(compact_result(result), ensure_ascii=False))
    if args.command == "audit-volume":
        audit = current_volume_audit(result)
        return 1 if audit is not None and audit["status"] == "block" else 0
    return 0
