...
def evaluate_wordcount(
    ...
    bands: dict[str, dict[str, int]] | None = None,
) -> dict[str, Any]:
    try:
    ...
    bands = bands or compute_wordcount_bands(target)
...
def target_from_outline(value: str) -> int:
    return contract_from_outline(value)["target"]


def contract_from_outline(value: str) -> dict[str, Any]:
    text = normalize_newlines(value)
    if text.startswith("\ufeff"):
        text = text[1:]
    targets = list(dict.fromkeys(_TARGET_LINE_RE.findall(text)))
    metrics = list(dict.fromkeys(_METRIC_LINE_RE.findall(text)))
    ranges = list(dict.fromkeys(_RANGE_LINE_RE.findall(text)))
    ...
    require(len(ranges) <= 1, "字数范围 must appear at most once")
    target = parse_target(targets[0])
    if ranges:
        low = parse_target(ranges[0][0])
        high = parse_target(ranges[0][1])
        require(low <= high, "字数范围 minimum cannot exceed maximum")
        band = {"min": low, "max": high}
        return {"target": target, "internal": band, "user": band, "explicit_range": True}
    bands = compute_wordcount_bands(target)
    return {"target": target, "internal": bands["internal"], "user": bands["user"], "explicit_range": False}
...
def build_project_wordcount_record(project: Path, chapter: int, *, resolution: str) -> dict[str, Any]:
    ...
    try:
        contract = contract_from_outline(outline_path.read_text(encoding="utf-8"))
