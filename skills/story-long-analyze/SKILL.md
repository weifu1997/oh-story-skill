...
你亲手做的**黄金三章深度拆解（Stage 1）、Stage 2 逐章处理、以及 Stage 3-6 的聚合/设定/报告/文风合成**，与 chapter-extractor 子代理遵循同一逻辑：拆的是用户**合法持有、拥有使用权**的虚构作品（已出版/连载或自有），性质是**只读的转化性文学批评**（编辑/评论/写作教学视角，不复制、不传播、不指导现实行为）。通俗题材里的家暴、出轨、复仇、暴力、情爱张力、黑暗伦理是常规虚构叙事元素，须照常做结构化提取，**不得以「敏感」「不适」为由拒绝整章或整本**——主线程拒绝会让整条自动化流水线停摆；个别片段无法处理则跳过、照常续完其余。
...
**预期耗时提示**：开始前根据章节数给用户一个粗估：<50 章通常 30-60 分钟；50-200 章通常 1-3 小时；>200 章可能需要多轮会话。Stage 2 逐章串行提取，Stage 3-6 仍依赖前序产物，需按阶段推进。
...
| 2 | 逐章摘要 | 分块章节文本 | 章节摘要.md（含情节点+角色+**关键信息与扩写技法**+**逐章写法公式**）。逐章写法公式必须提取情绪流向、节奏配比、结构公式、核心技巧、章尾卡点与伏笔。角色过滤（龙套不提取、别名归类）。每章10-40情节点（密度150-200字/个，按字数动态调节；公式低于10时仍按硬下限10拆足关键步骤）。**串行模式：每章 spawn 一个 chapter-extractor agent，同一时刻只跑 1 个**。**计数验证：摘要数 == 章节数，不等则标记失败章节**。 | 所有章节处理完成 |
...
| 4 | 设定+关系（4a/4b/4c） | **4a**：Stage 2 情节点+章节摘要（不依赖 Stage 3，但必须等 Stage 3 结束后再做）；**4b/4c**：Stage 3 合并后角色数据+情节点 | 设定/*.md + 角色/*.md。**4a 设定**（世界观/金手指/势力，从 Stage 2 mention 数据归纳）。**4b 角色完整档案**（两阶段模型：Stage 2 轻量提及 → Stage 4b 完整档案；别名解析置信度≥0.85自动合并）。**4c 角色关系提取**（从情节点提取，不从原文；含演变追踪+最终状态合并+隐含推断）。非人形反派在 4a 做完整抽象对抗型分析。 | 4a/4b/4c 全部完成 |
...
Stage 3（剧情聚合 + 角色合并）
              │
              ▼
Stage 4a（设定：世界观/金手指/势力）— 数据源是 Stage 2 摘要，不依赖 Stage 3 结论，但仍等 Stage 3 结束后再做
              │
              ▼
Stage 4b（角色完整档案）— 依赖 Stage 3 合并后的角色实体
              │
              ▼
Stage 4c（角色关系提取）— 依赖 4b 角色实体存在
```

禁止 Stage 3 与 4a 同时开工，也禁止为本阶段并行 spawn 多个子代理。4a 虽不读 Stage 3 产物，仍按固定顺序串行，避免瞬时打满中转配额。
...
Stage 2 使用 chapter-extractor agent **按章号顺序逐章处理**。同一时刻只允许 1 个 Agent 在跑。
...
- 按章号顺序一次只 spawn 1 个 `chapter-extractor`
- 必须等当前章完整返回、落盘并完成硬检查后，再 spawn 下一章
- 禁止一次发出多个 Agent 工具调用，禁止预启动或后台排队同时跑
- 每章完成后更新 `_progress.md` 记录已处理章节
...
- 单章失败不阻断管道；全部章节处理完成后才决定是否进入 Stage 3

### Agent 不可用降级

以下任一情况，Stage 2 不再 spawn `chapter-extractor`，改由主线程逐章处理（质量不受影响，只是少一层子代理）。**两条路径的要求是同一份**：主线程路径的概要写法、情节点白描、原文引用精选规则和输出自检都按 [output-templates.md](references/output-templates.md)「Stage 2 章节摘要+情节点」执行；上面的机械硬检查同样要跑。主线程路径没有 sonnet 升级重试这条路——硬检查命中时由主线程按失败项重写本章摘要 1 次，仍不过按 `⚠️ 跳过` 记入 `_progress.md` 「失败记录」表。
...
- `grep -cE '^\*\*概要\*\*' _章节摘要汇总.md` == 摘要文件数（`**概要**` 每章一行，chapter-extractor 输出与主线程摘要模板都有；不用 `## 第N章` 头——主线程摘要模板没有章节头，会误判）

Stage 3 / 4a / 4c / 散落情节兜底改为**只读一次 `_章节摘要汇总.md`** 并在上下文中复用，替代每阶段 `glob 章节/*_摘要.md` 重扫（同一份语料的 4-5 次冷读降为 1 次）。

**仅当语料能放进上下文时才生成汇总文件**：>500 章、或合并后 `_章节摘要汇总.md` 过大放不进上下文时**跳过本步骤**，改走 [material-decomposition.md](references/material-decomposition.md)「处理批次 → A. 子代理串行模式」：按 10-20 章/批 **一次只 spawn 1 个**子代理，子代理在自己上下文里读该批摘要、只回传 ≤8K tokens 的降维聚合，主线程等它返回后再开下一批、仅合并聚合结果（必要时分层两两合并）。**主线程不逐章读原始摘要**——跳过汇总文件不等于回到逐文件扫描，那对大书同样放不下。`_章节摘要汇总.md` 不替代 `章节/*_摘要.md`——单章文件仍是落盘真源，Stage 6 文风采样、人工复核照用单章文件。管道结束（Stage 6 后）删除 `_章节摘要汇总.md`——它是派生临时文件，不随 `拆文库/` 交付（`拆文库/` 会被 story-import 保留为写作工程）。
