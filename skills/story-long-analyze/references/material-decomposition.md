...
### 阶段 2：原子提取（串行 Agent 处理每章）

串行/降级策略见 SKILL.md「Stage 2 串行 Agent 策略」。
...
Stage 2 使用 chapter-extractor agent 串行处理（每章一个 agent，同一时刻只跑 1 个），不分块。
...
| >500 章 | **子代理串行批次**（见下「处理批次」）；语义分块只用于组织输出，不作为读取单位 | 10-20 章/批 |
...
Stage 3-5 的语料是 `章节/*_摘要.md`。**实测体积：中文章节摘要约 15KB/章 ≈ 5-7K tokens/章**（据大奉打更人 57 章样本，869,663 字节 / 57 ≈ 15,257 字节）。按这个体积，「把全书摘要一次读进主线程」在 >500 章时必然超上下文——所以大书不是"把块调大"，而是**换成子代理串行批次 + 降维聚合**。

**A. 子代理串行模式（>500 章默认，Stage 3-5 通用）**

1. **切批**：按章号顺序切成 10-20 章/批（≈5-15 万 tokens/批，落在子代理单次上下文内）。单章摘要明显更长时，按「一批不超过约 15 万 tokens」反推章数。批次边界尽量对齐语义块边界，对不齐时以章号连续为准。
2. **串行分派**：按批号顺序一次只 spawn 1 个子代理（`general-purpose` 或本阶段对应的专业 agent），把该批摘要文件路径交给它，**由子代理在自己的上下文里读原文**，主线程不读。必须等当前批完整返回后再 spawn 下一批；禁止一次发出多个 Agent 工具调用。
3. **降维回传**：子代理只回传该批的中间聚合，**每份 ≤8K tokens**，字段随阶段而定（剧情线候选 / 情节点归属 / 关键信息推进 / 情绪节奏 / 角色表）。禁止回传原始摘要或逐章罗列。
4. **主线程合并**：主线程只消费这些聚合结果做跨批合成。若批数过多、聚合总量仍超上下文，再做一层**两两/分组合并**（把 N 份聚合先并成 √N 份），直到能一次装下。合并同样一次只 spawn 1 个合并子代理。
...
无法 spawn 子代理时退回主线程顺序处理：**输入 6-8K token/块、章节边界对齐**，逐块读取并滚动累积中间结论（每块处理完只保留结论、丢弃原文）。这是「6-8K token/块」这条规格的**唯一适用场景**——它描述的是主线程单次读取量，不是大书的语义块尺度。主线程路径对 >500 章耗时很长，但结论质量与子代理串行批次一致。

**C. 不要做的事**

- 不要为了"塞进上下文"而把 50-200 章的语义块整块读进主线程——那是 25-150 万 tokens，必然失败。
- 不要把 6-8K token 当成大书的切批单位——914 章会切出 4000+ 批，聚合层数爆炸。
- 主线程在子代理串行批次模式下不读 `章节/*_摘要.md` 原文；一旦开始逐章读，预算模型就失效了。
