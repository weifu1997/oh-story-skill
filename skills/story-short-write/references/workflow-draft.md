...
**交付参数先锁定**：用户明确的字数范围优先，逐字取其最小值/最大值与节数；只给单一目标时用目标的 95%-105%。都未给时必须先告知将使用 8000-20000 字和大纲节数，并获得用户确认，不能从某平台经验偷偷推导。阈值见 `references/length-routing.md`。后文的默认字数不得覆盖用户范围。
...
写完每批后按 `short-format.md`「字数统计」检查整篇累计值与各节分布。某节明显短于相邻节时，先核对批准情节点、可见动作和后果是否已经完整；完整就保留，缺失才补回原计划内容。不得为拉齐节长新增冲突、对话、回忆、配角反应或独立事件。整篇以锁定的用户交付范围为准；未给范围时才使用 8000-20000 字默认值（见 `length-routing.md`）。
