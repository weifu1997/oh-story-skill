...
- **投稿成品形态**：标题 + 导语（不写"导语"二字，只留那几句）+ 数字分节的正文（含付费点断点）。投稿建议区间比 `length-routing.md` 的默认交付区间（8000-20000）更收敛：整篇约 8000-12000 字；分节数量与长度服从动作链、转场和付费断点，不设统一章长。投稿建议不得覆盖用户锁定范围。
