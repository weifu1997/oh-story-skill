...
缺 `_tracking-state.json` 时，先按 workflow-daily「首次初始化」处理：无正文初始化第 0 章，已有正文迁移；`check` 通过才读取上下文/组装 prompt。
...
10. **元信息扫描**：按步骤 6「正文元信息隔离」扫描标题行以外的正文，沿用其中的改写方式与故事内真实阅读/读者身份例外。
